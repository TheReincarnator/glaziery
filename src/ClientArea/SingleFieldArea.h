...
#ifndef __GLAZIERY_SINGLEFIELDAREA_H
#define __GLAZIERY_SINGLEFIELDAREA_H


namespace glaziery
{
	/**
	 * The single field area is a client area that has only one field,
	 * an arbitrary field, e.g. a scrolling field, such as a list, tree, or text field.
	 * The child field uses the whole client area's size.
	 * It is automatically resized together with the client area.
	 */
	class SingleFieldArea : public ClientArea
	{
		private:

			/**
			 * The content (child) field that uses the whole client area's space.
			 */
			Field * contentField;
		...
		protected:
			...
			/**
			 * Destroys the single field area.
			 */
			virtual ~SingleFieldArea();
		...
		public:
				...
				DECLARE_DYNAMIC(SingleFieldArea);
			#endif

		public:

			/**
			 * Creates a new single field area.
			 */
			SingleFieldArea();
